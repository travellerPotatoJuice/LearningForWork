...
+ 面向对象。封装，继承，多态。
  + 封装：把数据和操作数据的方法包装在一个类中，用访问控制符来控制对其内部数据的访问。
  + 继承：在父类的基础上进行扩展。
  + 多态：同一个接口或者父类的引用对象可以指向不同的实现类或者子类。让程序员可以面向抽象编程，而不是具体的实现类编程。
+ 平台无关性。一次编译，到处运行。
+ 多线程支持。便于实现并发编程。
+ 健壮性高。能自动处理异常情况。
+ 安全性高。有严格的访问控制和内存管理机制。
...
AOT（Ahead-of-Time Compilation），这种编译模式会在程序被执行前就直接将其编译成机器码，属于静态编译。Java9中引入了AOT

和JIT相比，**AOT有以下优点**：

+ 启动速度快。程序启动时无需进行即时解释，可以直接执行机器码，减少了启动时间
+ 运行性能稳定。不需要运行时动态编译，不会出现JIT编译所带来的性能波动
+ 运行时资源占用少。减少运行时JIT所占用的资源，适用于资源受限的设备（嵌入式，移动设备）
...
+ 编译时间长
+ 灵活性差。编译完成后，生成的机器码不能根据运行时的情况进行优化。
...
<font size=5>**包装类**</font>

包装类是将基本数据类型包装在一个类中的一种机制。

**重要机制**：

+ **自动装箱和自动拆箱**。自动装箱是将基本数据类型自动转换为对应的包装类对象，自动拆箱是将包装类对象自动转换为基本数据类型。写代码时尽量避免，因为会影响性能。
  + 自动装箱：调用了valueOf()方法
  + 自动拆箱：调用了xxxValue()方法
+ **缓存机制**。如果创建对象的范围在缓存之内，就不会创建新对象（用了new除外）
  + Byte，Short，Integer，Long这四种类型默认创建了【-128，127】的相应类型的缓存数据。
...
+ 静态变量在**类加载时**被创建，在整个程序运行期间都存在，保存在**方法区**。而**实例变量就保存在堆中**。
...
+ **成员变量有默认值**。如果没有给成员变量赋值，JVM会自动赋默认值。
  + 局部变量没赋值可以直接报错，成员变量的值可能是运行时赋的，不能直接报错。
  + 如果赋随机值，会提高程序调试成本。因为程序员不能根据成员变量的值判断其是否处于初始状态。
...
Java5开始，支持可变长参数，**在方法重载时会优先匹配固定参数**。



**参数传递方式**：

+ 值传递：传递的是实际参数的值的副本，参数变量被赋新值时只会修改副本的值不会影响原值。
+ 引用传递：方法调用时，传递的是实际参数的引用（即内存地址）。参数变量被赋予新的值时，会修改原始值的内容。对象类型采用引用传递方式传递参数变量的值。

**Java中只有值传递**。对于引用类型的对象，本身引用里存储的就是对象的地址，传递对象时是拷贝对象的地址给方法进行操作，对对象内部的操作会影响原来的对象，**但如果在方法内部尝试将引用副本的指向一个新的对象，不会影响原引用的指向**。
...
+ 设计目的
  + 接口：接口是为了对类进行约束，是自上而下的
  + 抽象类：抽象类是为了复用，是自下而上的

+ 继承和实现：

  + 接口：一个类可以实现多个接口
  + 抽象类：一个类只能继承一个接口（Java这么实现是为了防止菱形继承）

+ 方法：

  + 接口：Java8之前只能有抽象方法，Java8之后逐渐可以定义default，static， private修饰的非抽象方法。
  + 抽象类：可以包含抽象方法和非抽象方法。
...
泛型是一种编程范式，它允许在定义类、接口和方法时**不预先指定具体的类型**，而在**使用的时候再指定一个类型**。实际上，泛型指的就是“**参数化类型**”。
...
泛型**不支持基本数据类型**，只能支持对象类型（引用数据类型）。如果要用基本数据类型，就要改成使用对应的对象类型。
...
<font size=5>**上下界限定符**</font>

- 上界限定符：extends
  - 可以指定单个类或接口 / 多个接口 / 类和若干个接口
...
<font size=5>**泛型擦除**</font>

Java中的泛型被称为“伪泛型”，这是因为在编译器中，泛型信息会被擦除（Type Erasure），从而在生成的字节码中不再包含泛型的类型信息，但是指定了的类型是不会被擦除的。

例如：List<T> 泛型类在编译后会变成原始类List，而List<String>这种类型是会保留的

泛型擦除分为**无限制类型擦除**和**有限制类型擦除**。无限制类型擦除会直接将类型擦除为Object，有限制类型擦除会将类型擦除为指定的类型边界。
...
+ 可变性
  + String：不可变。String内部用byte[] value来存储字符串，该数组被声明为private（外界无法访问）， final（防止内部修改）的，并且没有向外提供修改数组的方法。String类被声明成final，导致外部无法继承，防止子类破坏String的不可变。
  + StringBuilder：可变
  + StringBuffer：可变
+ 线程安全性：
  + String：线程安全
  + StringBuilder：线程不安全
  + StringBuffer：线程安全，因为加了同步锁。
+ 性能
  + String：对String进行改变会创建新的String对象。对String调用“+”和“+=”进行拼接时实际上调的是**StringBuilder对象的append**方法，多次进行拼接则会创建多个StringBuilder对象，影响性能。
  + StringBuilder：每次对StringBuilder本身进行操作。性能比StringBuffer高一点，但是在多线程情况下不太安全。
  +  StringBuffer：每次对StringBuffer本身进行操作
...
+ Error：系统级别错误。如果没做特殊处理的话，发生Error后，JVM会选择终止线程
+ Exception：异常。程序出现的问题
  + Checked Exception：受检异常，也叫编译时异常。
  + Unchecked Exception：非受检异常，也叫运行时异常。
...
+ 不要把异常定义为静态变量。静态变量在类的生命周期中只存在一份，当不同的错误情况先后发生时，这个静态异常变量会被不断地复用和覆盖，导致难以定位异常发生的真正位置。
+ 避免重复记录日志。在最外层捕获并记录异常，内层只需要抛出就可以了。
  + 比如说A调用B，B调用C。当C出现异常时，直接让C和Bthrows异常即可，让A来catch异常，并catch体里输出日志。这样可以有效避免日志的重复记录。
+ 尽量捕获特定的异常。不要捕获类似于Exception这样的通用异常，这不便于排查。
+ 不要在finally块中处理返回值或者直接return。try中如果有return语句，JVM会先暂存return内容，等执行完finally中的语句再return。如果finally里有return语句，就会影响try的返回值。
...
反射 (Reflection) 是 Java 的特征之一，是Java提供的能在运行期获取对象信息的能力。
...
+ 性能开销大。反射和直接调用方法比，需要在运行时解析类信息，还涉及额外的安全检查，造成开销。尤其是在高并发的场景下【可以通过缓存获得到的Method进行优化】
+ 带来安全问题。反射可以绕过Java的访问控制机制，会导致安全漏洞。
...
<font size=5>**认识注解**</font>

Annotation

Java中的**特殊标记**，让其他程序可以根据注解信息来决定怎么执行该程序
...
+ RetentionPolicy.SOURCE：注解只在源代码中存在，不会保留在字节码文件中。用于给编译器提供一些提示信息。
+ RetentionPolicy.CLASS：注解在字节码文件中存在，但运行时不会加载到JVM中。用于在编译阶段进行一些检查和处理。
+ RetentionPolicy.RUNTIME：注解在运行时仍然可以通过反射读取到。用于在运行时根据注解来动态地决定程序的行为。
...
**序列化**：将**对象**转换成可以存储或传输的形式的过程。

**反序列化**：序列化的逆过程。将存储的字节流或从网络接收到的字节流转换回对象的过程。

**transient**：

+ 只能修饰变量，不能修饰类
+ transient修饰的变量，反序列化后会变成默认值
+ static修饰的变量不属于对象，所以不会被序列化



JDK自带序列化方法，但是它不支持跨语言调用，并且性能较差，还存在安全问题，因此一般不推荐使用。比较常用的序列化协议有 Hessian、Kryo、Protobuf、ProtoStuff
